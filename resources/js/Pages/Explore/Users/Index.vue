<template>
    ...
    <AuthenticatedLayout>
        <div class="max-w-lg w-full mb-12">
            ...
            <div class="h-screen overflow-y-scroll no-scrollbar m-4 mb-12 lg:mb-0">
                ...
                <div class="mx-auto">
                    <div v-for="(user, index) in users" :key="index" class="p-2">
                        <div class="p-2 flex flex-col lg:flex-row justify-between">
                            <div class="flex items-center">
                                <img :src="user.avatar" @error="$event.target.src='/image/abc.png'" width="50" height="50" class="rounded-full">
                                <p class="ml-2 text-md hover:underline font-bold shrink-0"><Link :href="route('user.profile', user)">{{ user.name }}</Link></p>
                                ...
                                <p class="ml-2 text-sm shrink-0">@{{ user.username }}</p>
                            </div>
                            <div class="ml-5">
                                <FollowButton v-if="$page.props.auth.user" class="ml-10" :userId="user.id" :follows="user.follows" v-show="user.id != $page.props.auth.user.id"/>
                            </div>
                        </div>
                    </div>
                </div>
